...
def embedding_rnn_seq2seq(encoder_inputs,
  ...
  with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq") as scope:
    ...
    conat_encoder_state = tf.concat(1, encoder_state)

    with tf.variable_scope('encoder_to_latent'):
      with tf.variable_scope('mean'):
        w = tf.get_variable("w",[embedding_size, latent_dim],
          dtype=dtype)
        b = tf.get_variable("b", [latent_dim], dtype=dtype)
        enc_mean = tf.nn.relu(tf.matmul(conat_encoder_state, w) + b)
      with tf.variable_scope('stddev'):
        w = tf.get_variable("w",
          [embedding_size, latent_dim], dtype=dtype)
        b = tf.get_variable("b", [latent_dim], dtype=dtype)
        enc_stddev = tf.nn.relu(tf.matmul(conat_encoder_state, w) + b)

    batch_size = tf.shape(enc_stddev)[0]
    epsilon = tf.random_normal([batch_size, latent_dim])
    input_sample = enc_mean + tf.mul(epsilon, enc_stddev)

    with tf.variable_scope('latent_to_decoder'):
      with tf.variable_scope('cell_state'):
        w = tf.get_variable("w", [latent_dim, embedding_size],
          dtype=dtype)
        b = tf.get_variable("b", [embedding_size], dtype=dtype)
    ...
    dec_initial_state = [dec_initial_state_c, dec_initial_state_h]
...
def model_with_buckets(encoder_inputs, decoder_inputs, targets, weights,
  ...
  def encoder_with_buckets(encoder_inputs, buckets, encoder, name=None):
  """Create a sequence-to-sequence model with support for bucketing.
  ...
  means = []
  logvars = []
  with ops.name_scope(name, "encoder_with_buckets", all_inputs):
    for j, bucket in enumerate(buckets):
      with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                                         reuse=True if j > 0 else None):
