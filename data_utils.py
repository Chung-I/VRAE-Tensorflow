...
def get_cts_train_set(directory):
  """Download the WMT en-fr training corpus to directory unless it's there."""
  train_path = os.path.join(directory, "giga-fren.release2.fixed")
  if not (gfile.Exists(train_path +".fr") and gfile.Exists(train_path +".en")):
    corpus_file = maybe_download(directory, "training-giga-fren.tar",
...
def poem_to_lines(poem):
  return re.split("[\uff0c\u3002]+", re.sub("[\n]", "", poem))[:-1]

def basic_tokenizer(sentence):
  """Very basic tokenizer: split the sentence into a list of tokens."""
  return [w for w in re.sub("[\n]", "", sentence)]
...
def data_to_lines(raw_train_path_in, used_train_path_in, used_train_path_out, tokenizer):
...
def ST_data_to_lines(raw_train_path_in, raw_train_path_out,
        used_train_path_in, used_train_path_out, tokenizer):
      ...
      with gfile.GFile(used_train_path_out, mode="w") as out_file:
        for poem in raw_file:
          ...
          for idx, _ in enumerate(lines):
            if idx > 0:
              in_file.write(lines[idx] + "\n")
              out_file.write(lines[idx - 1] + "\n")
            in_file.write(lines[idx] + "\n")
...
def prepare_poem_data(data_dir, in_vocabulary_size, out_vocabulary_size, line_based=False, skip_thought=False, tokenizer=None):
  """Get poem data into data_dir, create vocabularies and tokenize data.
  ...
  raw_train_path_in = train_path + ".in"
  raw_train_path_out = train_path + ".out"
  used_train_path_in = raw_train_path_in
  used_train_path_out = raw_train_path_out
  if line_based:
    if skip_thought:
      used_train_path_in = train_path + ".lineST.in"
      used_train_path_out = train_path + ".lineST.out"
      ST_data_to_lines(raw_train_path_in, raw_train_path_out,
        used_train_path_in, used_train_path_out, tokenizer)
    else:
      used_train_path_in = train_path + ".line.in"
      used_train_path_out = train_path + ".line.out"
      data_to_lines(raw_train_path_in, used_train_path_in, used_train_path_out, tokenizer)
  ...
  create_vocabulary(in_vocab_path, raw_train_path_in, in_vocabulary_size, tokenizer)
  create_vocabulary(out_vocab_path, raw_train_path_out, out_vocabulary_size, tokenizer)
  ...
  data_to_token_ids(used_train_path_in, in_train_ids_path, in_vocab_path, tokenizer)
  data_to_token_ids(used_train_path_out, out_train_ids_path, out_vocab_path, tokenizer)
